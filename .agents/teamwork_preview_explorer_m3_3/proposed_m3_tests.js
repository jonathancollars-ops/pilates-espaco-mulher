/**
 * Proposed Test Suite: tests/m3_patient_integration.test.js
...
require(path.join(rootDir, 'tests', 'mock-rn.cjs'));
require('tsx/cjs');
...
const {
  patientRepository,
} = require(path.join(rootDir, 'src', 'database', 'repositories', 'patientRepository.ts'));
const {
  formatPhone,
  cleanDigits,
  formatDateBR,
  parseBRDateToISO,
  calculateAge,
} = require(path.join(rootDir, 'src', 'utils', 'formatters.ts'));
const { CLINIC_IDENTITY } = require(path.join(rootDir, 'src', 'design-system', 'ClinicIdentity.tsx'));
...
describe('Milestone 3 — App Startup, Navigation & Reactive Patient Integration', () => {
  ...
  describe('1. Zero-Login Startup & Clinical Identity', () => {
    ...
    test('Zero login screens: Navigation mounts directly on Pacientes tab as root initialRoute', () => {
      // Direct launch validation: no auth provider or login route
      const initialTab = 'Pacientes';
      assert.strictEqual(initialTab, 'Pacientes');
    });
  });
