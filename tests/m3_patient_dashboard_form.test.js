/**
 * Test Suite: tests/m3_patient_dashboard_form.test.js
 * Pilates Espaço Mulher — Dra. Rogéria Collares (CREFITO 23093-F)
 * 
 * Milestone 3: Patient Form & Validation Logic, Formatting Masks,
 * Default Values, and Negative Constraint Verifications.
 */
...
describe('M3 Patient Form & Validation Logic Suite', () => {
  ...
  describe('1. Authoritative Negative Constraints (Strict Exclusion)', () => {
    test('Ensures CPF is strictly absent from PatientFormModal code', () => {
    ...
    test('Ensures Patient entity types strictly exclude CPF, Estado Civil, and CEP', () => {
      const typesSource = fs.readFileSync(
        path.join(projectRoot, 'src', 'types', 'patient.ts'),
        'utf8'
      );
      const nonCommentCode = typesSource
        .split('\n')
        .map((line) => line.replace(/\/\/.*$/, ''))
        .filter((line) => !line.trim().startsWith('*') && !line.trim().startsWith('/*'))
        .join('\n');
      assert.equal(nonCommentCode.match(/\bcpf\b/i), null);
      assert.equal(nonCommentCode.match(/estado[\s_]?civil/i), null);
      assert.equal(nonCommentCode.match(/\bcep\b/i), null);
    });
  });
  ...
  describe('2. Required Field Validations', () => {
    function validatePatientForm(data) {
      ...
      // Name validation (required, >= 3 chars)
      ...
      // Phone validation (required, 10 or 11 digits)
      ...
      // Birthdate validation (optional, but valid date if present)
      ...
      // Age validation (optional, 0 to 125)
